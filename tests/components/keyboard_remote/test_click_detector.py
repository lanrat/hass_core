...
from .conftest import mock_monotonic
...
async def test_double_click(hass: HomeAssistant, fire_event: MagicMock) -> None:
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 0.1, 0.2, 0.3)):
...
async def test_long_click_at_boundary(
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 0.4)):
        detector.on_key_down(30)
        detector.on_key_up(30)
...
async def test_hold_too_long(hass: HomeAssistant, fire_event: MagicMock) -> None:
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 5.0)):
        detector.on_key_down(30)
        detector.on_key_up(30)
...
async def test_multiple_keys_independent(
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 0.0, 0.1, 1.0)):
        detector.on_key_down(30)
...
async def test_unexpected_key_down_resets(
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 1.0, 1.1)):
        detector.on_key_down(30)
...
async def test_second_press_held_long_fires_double_click(
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 0.1, 0.2, 5.0)):
        detector.on_key_down(30)
...
async def test_long_click_at_max_boundary(
    ...
    with patch("time.monotonic", side_effect=mock_monotonic(0.0, 3.0)):
        detector.on_key_down(30)
        detector.on_key_up(30)
